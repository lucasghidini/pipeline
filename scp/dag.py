        ...
        def load_incremental_data(table_name: str, max_id: int):
            with PostgresHook(postgres_conn_id='postgres').get_conn() as pg_conn:
                with pg_conn.cursor as pg_cursor:
                    ...
                    collumns = [row[0]for row in pg_cursor.fetchall()]
                    collumns_list_str = ', '.join(collumns)
                    placeholders = ','.join(['%s'] * len(collumns))   

                    pg_cursor.execute(f"SELECT {collumns_list_str} FROM {table_name} WHERE {primary_key} > {max_id,} ")
                    rows = pg_cursor.fetchall()

                    with SnowflakeHook(snowflake_conn_id='snowflake').get_conn() as sf_conn:
                        with sf_conn.cursor() as sf_cursor:
                            insert_query = f"INSERT INTO {table_name} ({collumns_list_str}) VALUES ({placeholders})"
                            for row in rows:
